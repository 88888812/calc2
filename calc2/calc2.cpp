...
bool ERROR(std::string a)// для проверки ошибок
{
    ...
    for (unsigned int k = 0; k < a.size(); k++)
    {
        if (a[k] == 's' && k != a.size() - 2 && (a[k + 1] != 'i' || a[k + 2] != 'n')) return false; // Тут и след два ошибка в
        ...
        if (a[k] == '(' && k != a.size() - 2 && (a[k + 1] == '-' || a[k + 1] == '+' || a[k + 1] == '/' || a[k + 1] == '*')) return false; // что после скобки не идет действие
        // для унарного минуса не будет работать тк перезапишется к этому моменту в вид: 0-a
        if (a[k] == ')' && k != 0 && (a[k - 1] == '-' || a[k - 1] == '+' || a[k - 1] == '/' || a[k - 1] == '*')) return false; // что перед закрытием действие
        if (a[k] == '(' && k != a.size() - 2 && (a[k + 1] == '-' || a[k + 1] == '+' || a[k + 1] == '/' || a[k + 1] == '*')) return false;
        if (a[k] == ')' && k != a.size() - 1 && (a[k + 1] != '-' || a[k + 1] != '+' || a[k + 1] != '/' || a[k + 1] != '*')) return false;
    }
    if (kolvo_open_skobok != kolvo_close_skobok) return false;
    return true;
}
int main()
{   // буду использовать систему приоритета
    // + - 0 баллов
    // * /  1 балл
    ...
    std::cout << "Калькулятор, можете посчитать такие результаты для операций : +-*/ ,а также унарный минус " << std::endl;
    ...
    if (!ERROR(expression))
    {
        ...
        {
            if (vec_location[o].what_do == ')') { vec_location.erase(vec_location.begin() + o); o--; }
            if (vec_location[o].what_do == '(') { vec_location.erase(vec_location.begin() + o); o--; }
        }
        for (unsigned int o = 0; o < vec_priority.size(); o++)
        {
            if (vec_priority[o].what_do == '(') { vec_priority.erase(vec_priority.begin() + o); o--; }
            if (vec_priority[o].what_do == ')') { vec_priority.erase(vec_priority.begin() + o); o--; }
        }
            ...
            for (unsigned int m = 0; m < vec_location.size(); m++) {
                if (vec_priority[p].location == vec_location[m].location) // для подбора чисел для операции
                {
                    ...
                    case '*':
                        ...
                        vec_priority.erase(vec_priority.begin() + p); // удаляем тут и в векторе по позициям это действие
                    ...
                    case '-':
                        ...
                        break;
                    case '/':
                        if (chisla[m + 1] == 0) { er++; vec_priority.clear(); }
                        else {
